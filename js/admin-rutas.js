...
(function () {
  ...
  async function renderEditor(route){
    ...
    if(routePlaces.length){list.innerHTML=editorMarkup({...current,id:route?.id},false);renderStops();return;}
    list.innerHTML=editorMarkup({...current,id:route?.id},true);renderStops();
    try{await loadRoutePlaces();list.innerHTML=editorMarkup({...current,id:route?.id},false);renderStops();}
    catch(error){console.error('No se pudieron cargar las paradas para rutas:',error);list.innerHTML=editorMarkup({...current,id:route?.id},false,'No fue posible leer la colección de paradas.');renderStops();}
  }

  function renderPreview(){
    const box=document.getElementById('touristRoutePreview');if(!box)return;
